...
agg_trans = pd.read_sql(agg_trans_query, con)
agg_users = pd.read_sql(agg_users_query, con)
map_trans = pd.read_sql(map_trans_query, con)
map_users = pd.read_sql(map_users_query, con)
top_trans = pd.read_sql(top_trans_query, con)
top_users = pd.read_sql(top_users_query, con)
...
with st.sidebar:
  selected = option_menu(menu_title=None, options=["All over India wise","State Data","Top 10 wise"], icons=["clipboard-data","award","capslock-fill","coin"], orientation="horizontal")
